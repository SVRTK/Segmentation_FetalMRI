...
import pandas as pd
from src.constants import FOLDER_PATH_3D_DATA as FOLDER_PATH_DATA
...
TEST_SUBJs = ['res-237', 'res-266', 'res-306']
...
for id_folder, folder_name in enumerate(sorted(glob.glob(FOLDER_PATH_DATA + '/*'))):
    # print folder name
    print('===> ' +
          str(id_folder) + ' ' + folder_name + '/stack-files: ' +
          str(len(sorted(glob.glob(folder_name + '/stack-files/3d*stack*')))))

    # Choose an id to keep for validate
    id_valid = id_folder % (len(sorted(glob.glob(folder_name + '/mask-files/3d*mask-01*'))) + 1)
    print('     - Valid_ID:', id_valid)

    # Go through all the files (mask + mri) in each folder
    for id_file, (file_name_mask1, file_name_mask2, file_name_mask3, file_name_stack) in enumerate(zip(
            sorted(glob.glob(folder_name + '/mask-files/3d*mask-01*')),
            sorted(glob.glob(folder_name + '/mask-files/3d*mask-02*')),
            sorted(glob.glob(folder_name + '/mask-files/3d*mask-03*')),
            sorted(glob.glob(folder_name + '/stack-files/3d*stack*')))):

        print('     - mask1' + file_name_mask1)
        print('     - mask2' + file_name_mask2)
        print('     - mask3' + file_name_mask3)
        print('     - stack' + file_name_stack)

        # Read data:
        img_current = nib.load(file_name_stack).get_data().astype(np.float32)
        msk1_current = nib.load(file_name_mask1).get_data().astype(np.float32)
        msk2_current = nib.load(file_name_mask2).get_data().astype(np.float32)
        msk3_current = nib.load(file_name_mask3).get_data().astype(np.float32)
        img_aff = nib.load(file_name_stack).get_affine()
        lab_aff = nib.load(file_name_mask1).get_affine()
        print('     - ', img_current.shape)
        print('     - ', msk1_current.shape)
        print('     - ', msk2_current.shape)
        print('     - ', msk3_current.shape)

        # Create csv file for training
        current_row = [file_name_stack.split('/')[x] for x in [-3, -2, -1]]
        current_row = '/'.join([current_row[x] for x in [0, 1, 2]]) + ','

        masks_list = [file_name_mask1, file_name_mask2, file_name_mask3]

        for id_type, type_ in enumerate(masks_list):
            current_row_temp = [type_.split('/')[x] for x in [-3, -2, -1]]
            if id_type == len(masks_list) - 1:
                current_row_temp = '/'.join([current_row_temp[x] for x in [0, 1, 2]])
            else:
                current_row_temp = '/'.join([current_row_temp[x] for x in [0, 1, 2]]) + ','
            current_row += current_row_temp

        # If brain-i in TEST_SUBJs, append to test csv
        if file_name_stack.split('/')[-3] in TEST_SUBJs:
...
for data_type, data_csv in zip(['train', 'valid', 'test'],
    ...
    id_header = 0
    with open('/data/project/Localisation/data/data_localisation_3labels_' + data_type + '.csv', 'w') as f:
        if id_header == 0:
            f.write("t2w,lab1,lab2,lab3\n")
            id_header += 1
